...
// When YouTube bot-walls the server's IP ("Sign in to confirm you're not a
// bot"), cookies from a signed-in browser get through. Set YT_COOKIES_TXT to
// the contents of a Netscape-format cookies.txt export for youtube.com.
let ytDlpCookiesFile = null;
if (process.env.YT_COOKIES_TXT) {
  try {
    ytDlpCookiesFile = path.join(os.tmpdir(), 'yt-cookies.txt');
    writeFileSync(ytDlpCookiesFile, process.env.YT_COOKIES_TXT, { mode: 0o600 });
    console.log('extract: yt-dlp cookies loaded from YT_COOKIES_TXT');
  } catch (err) {
    console.error('extract: could not write cookies file:', err.message);
    ytDlpCookiesFile = null;
  }
}

// Escape hatch for tuning yt-dlp on a deployed instance without a code
// change, e.g. YTDLP_EXTRA_ARGS="--extractor-args youtube:player_client=tv"
const YTDLP_BASE_ARGS = [
  ...(ytDlpCookiesFile ? ['--cookies', ytDlpCookiesFile] : []),
  ...(process.env.YTDLP_EXTRA_ARGS ? process.env.YTDLP_EXTRA_ARGS.split(/\s+/) : []),
];
...
async function viaYtDlp(url) {
  ...
  if (!ytDlp) throw new Error('binary not installed');
  const bin = ytDlp.path;

  const json = await new Promise((resolve, reject) => {
    execFile(
      bin,
      [...YTDLP_BASE_ARGS, '--no-playlist', '--format', YTDLP_FORMAT, '--dump-single-json', '--no-warnings', url],
  ...
  return {
    ...
    createStream: async () => {
      const proc = spawn(
        bin,
        [...YTDLP_BASE_ARGS, '--no-playlist', '--format', json.format_id || YTDLP_FORMAT, '--output', '-', '--no-warnings', '--quiet', url],
...
// Runs every extractor against a URL and reports each outcome — surfaced by
// /api/diag so a deployed instance can be debugged without log access.
async function diagnose(url) {
  ...
  for (const attempt of [viaYtDlp, viaYtdl, viaInnertube]) {
    ...
    try {
      const track = await attempt(url);
      await probeStream(track);
      results.push({ extractor: attempt.name, ok: true, source: track.source, ms: Date.now() - started });
    } catch (err) {
      results.push({ extractor: attempt.name, ok: false, error: err.message, ms: Date.now() - started });
    }
  }
  return results;
}

module.exports = { extract, diagnose, ytDlpInfo: getYtDlpBin, validateURL: ytdl.validateURL };
